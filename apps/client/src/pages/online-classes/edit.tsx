...
export const OnlineClassEdit: React.FC<IResourceComponentsProps> = () => {
  const { sendUpdateEvent } = useCrudSocketClient<IOnlineClass>({
  ...
  useEffect(() => {
    if (initialValues) {
      setSelectedSubjectId(initialValues.subjectId);
      setSelectedChapterId(initialValues.chapterId);
      setSelectedTopicId(initialValues.topicId);
    }
  }, [initialValues]);
  ...
  return (
    <Create saveButtonProps={saveButtonProps}>
      <Form
        {...formProps}
        layout="vertical"
        initialValues={initialValues ?? { isEnded: false, isActive: true }}
      >
        <Form.Item
          label="Subject"
          name={"subjectId"}
