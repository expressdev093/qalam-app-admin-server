...
import { HttpError, UpdateResponse, useApiUrl } from "@refinedev/core";
import {
  Form,
  Input,
  Modal,
...
export const AppCornerEdit: React.FC = () => {
  const apiUrl = useApiUrl();
  const { sendUpdateEvent } = useCrudSocketClient<IAppCorner>({
  ...
  useEffect(() => {
    if (record) {
      ...
      form.setFieldValue("video", Utils.getFileList(record.video));
      form.setFieldValue(
        "videoThumbnail",
        Utils.getFileList(record.videoThumbnail)
      );
    }
  }, [record]);
